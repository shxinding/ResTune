...
from .utils.parser import get_action_data_from_res_cpu
...
def load_source_model(load_dir_tps, load_dir_cpu, load_dir_latency):
    ...
    for w in workload_list:
        ...
        model_path_latency = os.path.join(load_dir_latency, w)
        model_latency = torch.load(model_path_latency)
        model_dir[w] = (model_tps, model_cpu, model_latency)
    return model_dir
...
def compute_ranking_loss(f_samps, target_y):
    ...
    if f_samps.ndim == 4:
        ...
        cartesian_y_latency = torch.cartesian_prod(
            target_y[:, 2].squeeze(-1),
            target_y[:, 2].squeeze(-1),
        ).view(n, n, 2)
        ...
        rank_loss = (((f_samps[:,:,:,0].diagonal(dim1=1, dim2=2).unsqueeze(-1) < f_samps[:,:,:,0]) ^ (
                ...
                ((f_samps[:, :, :, 2].diagonal(dim1=1, dim2=2).unsqueeze(-1) < f_samps[:, :, :, 2]) ^ (
                             cartesian_y_latency[..., 0] < cartesian_y_latency[..., 1]))
                 ).sum(dim=-1).sum(dim=-1)
        ...
        for i in range(1, target_y.shape[0]):
            rank_loss +=   (((roll_col(f_samps[:,:,0], i) < f_samps[:,:,0]) ^ (roll_col(y_stack[:,:,0], i) < y_stack[:,:,0])) \
               | ((roll_col(f_samps[:, :, 1], i) < f_samps[:, :, 1]) ^ (roll_col(y_stack[:, :, 1], i) < y_stack[:, :, 1]))
               | ((roll_col(f_samps[:, :, 2], i) < f_samps[:, :, 2]) ^ (roll_col(y_stack[:, :, 2], i) < y_stack[:, :, 2]))
                            ).sum(dim=-1)
    return rank_loss
...
def compute_rank_weights(train_x, train_y, base_models, target_model, num_samples):
    ...
    for task in list(base_models.keys()):
        ...
        posterior_latency = model[2].posterior(train_x)
        sampler = SobolQMCNormalSampler(num_samples=num_samples)
        base_f_samps_latency = sampler(posterior_latency).squeeze(-1).squeeze(-1)
        base_f_samps = torch.stack((base_f_samps_tps, base_f_samps_cpu, base_f_samps_latency), -1)
        # compute and save ranking loss
        try:
            ranking_losses.append(compute_ranking_loss(base_f_samps, train_y).numpy())
        except:
            pdb.set_trace()
            compute_ranking_loss(base_f_samps, train_y)
    ...
    target_f_samps_latency = get_target_model_loocv_sample_preds(train_x, train_y[:, 2].reshape(-1, 1), target_model[2],num_samples)
    target_f_samps = torch.stack((target_f_samps_tps, target_f_samps_cpu, target_f_samps_latency), -1)
...
def initialize_WGPE_model_multiOutput(train_x, train_y, load_dir_tps, load_dir_cpu, load_dir_latency, tps_constrained, latency_constrained):
    ...
    target_model_latency = get_fitted_model(train_x, train_y[:, 2].reshape(-1, 1))
    target_model = (target_model_tps, target_model_cpu, target_model_latency)
    source_model_dir = load_source_model(load_dir_tps, load_dir_cpu, load_dir_latency)
    ...
    model_list_latency = []
    for task in list(source_model_dir.keys()):
        model_list_latency.append(source_model_dir[task][2])
    model_list_latency.append(target_model[2])
    ...
    if map_num == 1:
        ...
        model_latency = model_list_latency[non_zero_weight_indice]
        model = ModelListGP(model_tps, model_cpu, model_latency)
        tps_constrained = (tps_constrained - model_tps.Y_mean) / model_tps.Y_std
        latency_constrained = (latency_constrained - model_latency.Y_mean) / model_latency.Y_std
        return model, True, (tps_constrained.detach().numpy().flatten()[0], latency_constrained.detach().numpy().flatten()[0])

    else:
        wgpe_model_tps = WGPE(model_list_tps, rank_weights)
        wgpe_model_cpu = WGPE(model_list_cpu, rank_weights)
        wgpe_model_latency = WGPE(model_list_latency, rank_weights)
        model =  ModelListGP(wgpe_model_tps, wgpe_model_cpu, wgpe_model_latency)
        return model, False, None

def initialize_WGPE_model_meta(train_x, train_y, workload_target, load_dir_tps, load_dir_cpu, load_dir_latency):
    try:
        target_model_tps = get_fitted_model(train_x, train_y[:, 0].reshape(-1, 1))
        target_model_cpu = get_fitted_model(train_x, train_y[:, 1].reshape(-1, 1))
        target_model_latency = get_fitted_model(train_x, train_y[:, 2].reshape(-1, 1))
    ...
    if target_model:
        ...
        model_list_latency.append(target_model_latency)
